...
class Transport:
    ...
    def send_within(self, packet: Packet, player_id, delay: float):
        ...
        self.logger.info(
            f"DELAY_INFO {self.myself} to {player_id} | send_time:{now} | delay_time: {now+delay} | packet_type: {packet.get_packet_type()}")
        ...
        try:
            ...
            if data:
                ...
                if packet.get_packet_type()=="action":
                    self.logger.info(
                    f"ACTION_PACKET_INFO Length: {length} | Packet Type: {packet.get_packet_type()} | Data: {packet.get_data()} | RTT: {rtt} | Throughput: {throughput}****")
                else:
                    self.logger.info(
                        f"PACKET_INFO Length: {length} | Packet Type: {packet.get_packet_type()} | RTT: {rtt} | Throughput: {throughput}")
                return packet
        except Empty:
            return
